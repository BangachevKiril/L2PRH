...
#SBATCH --array=0-35
#SBATCH -o logs/corr_diag_%A_%a.out
#SBATCH -e logs/corr_diag_%A_%a.err
...
RAW_STD_METRICS = {
    "weighted_correlation": "weighted_correlation_std",
    "binary_correlation": "binary_correlation_std",
}
...
for dataset_name, dataset_title in datasets:
    for dimension in dimensions:
        for sparsity_pattern, sparsity_title in sparsity_patterns:
...
expected_total = len(datasets) * len(dimensions) * len(sparsity_patterns) * len(METRICS) * len(panels)
if n_total != expected_total:
    print(f"Warning: expected {expected_total} jobs, got {n_total}")
...
for local_idx, job in enumerate(my_jobs, start=1):
    ...
    sparsity_title = job["sparsity_title"]
    metric_key = job["metric_key"]
    metric_title = job["metric_title"]
    panel = job["panel"]
    panel_name = job["panel_name"]
    raw_std_metric = RAW_STD_METRICS.get(metric_key)

    filtered_dir = os.path.join(
        PRH_DATA_ROOT,
        f"topk_sae_{dataset_name}_correlations",
        f"d_{dimension}_{sparsity_pattern}",
    )
    raw_dir = os.path.join(
        PRH_DATA_ROOT,
        f"topk_sae_{dataset_name}_correlations",
        f"d_{dimension}_{sparsity_pattern}_rand_baseline_summary",
    )

    savepath = os.path.join(
        OUT_DIR,
        f"{metric_key}_{dataset_name}_{dimension}_{sparsity_pattern}_{panel}.pdf",
    )
    ...
    print(f"  sparsity        = {sparsity_pattern}")
    print(f"  metric_key      = {metric_key}")
    print(f"  raw_std_metric  = {raw_std_metric}")
    print(f"  panel           = {panel}")
