...
#SBATCH --array=0-23
#SBATCH -o logs/signal_diag_%A_%a.out
#SBATCH -e logs/signal_diag_%A_%a.err
...
OUT_DIR="/home/kirilb/data/L2PRH/signal_diagrams"
...
OUT_DIR = os.environ.get("OUT_DIR", "/home/kirilb/data/L2PRH/signal_diagrams")
...
EDIT_METRICS = {"knn_edit_10", "knn_edit_100"}
...
if n_total != 144:
    print(f"Warning: expected 144 jobs, got {n_total}")
...
for local_idx, job in enumerate(my_jobs, start=1):
    ...
    raw_dir = os.path.join(PRH_DATA_ROOT, f"metrics_embedded_{dataset_name}_centered")
    filtered_dir = os.path.join(
        PRH_DATA_ROOT,
        f"topk_metrics_embedded_{dataset_name}",
        f"weighted_d_{dimension}_kvar_truncated",
    )
    savepath = os.path.join(
        OUT_DIR,
        f"{metric_key}_{dataset_name}_{panel}_{dimension}.pdf",
    )

    display_panel_name = panel_name
    if panel == "diff" and metric_key in EDIT_METRICS:
        display_panel_name = reverse_hyphen_phrase(panel_name)
    ...
    print(f"  panel         = {panel}")
    print(f"  title_panel   = {display_panel_name}")
    ...
    panel=panel,
    title=f"{display_panel_name} {metric_title} Alignment over {dataset_title}",
    savepath=savepath,
    type_name=type_name,
